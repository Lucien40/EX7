...
class U2 {
 public:
  ...
  virtual double operator()(
      double const& x) const = 0;  // Evalue u^2 au point x
...
class U2_const : public U2 {
 public:
  ...
  double operator()(double const& x) const { return u2; }

  double max() const { return u2; }
 ...
 public:
  ...
  double operator()(double const& x) const {
    if (x < xa)
      ...
      return g * (h_ocean + (h_recif - h_ocean) *
                                pow(sin(.5 * M_PI * (x - xa) / (xb - xa)), 2));
    else if (x < xc)
      return g * h_recif;
    else if (x < xd)
      return g * (h_recif - (h_recif - h_ocean) *
                                pow(sin(.5 * M_PI * (xc - x) / (xc - xd)), 2));
    else
      return g * h_ocean;
  }

  double max() const { return g * std::max(h_ocean, h_recif); }
...
ostream& operator<<(ostream& o, vector<T> const& v) {
  ...
  for (unsigned int i(0); i < (len - 1); ++i) o << v[i] << " ";
...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1)  // Fichier d'input specifie par l'utilisateur ("./Exercice7
                 // config_perso.in")
    inputPath = argv[1];
  ...
  for (int i(2); i < argc; ++i)  // Input complementaires ("./Exercice7
  ...
  // Conditions aux bords (les strings sont converties en valeurs numeriques a
  // l'aide d'un enumerateur) :
  ...
  if (cb == "fixe")
    ...
    cerr << "Merci de choisir cb_gauche="
            "fixe"
            ", "
            "libre"
            ", "
  ...
  if (cb == "fixe")
    ...
    cerr << "Merci de choisir cb_droit="
            "fixe"
            ", "
            "libre"
            ", "
  ...
  if (cb_gauche == harmonique || cb_droit == harmonique || cb_gauche == pulse ||
      cb_droit == pulse) {
